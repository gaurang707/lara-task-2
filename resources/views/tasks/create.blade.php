                            ...
                            <div class="flex items-center space-x-6">
                                <label class="flex items-center space-x-2">
                                    <input type="radio" name="status" value="pending"
                                        class="text-blue-500 focus:ring-blue-400" @if(old('status', 'pending') == 'pending') checked @endif>
                                    <span>Pending</span>
                                </label>

                                <label class="flex items-center space-x-2">
                                    <input type="radio" name="status" value="completed"
                                        class="text-blue-500 focus:ring-blue-400" @if(old('status') == 'completed') checked @endif>
                                    <span>Completed</span>
                                </label>
                            </div>
                            @error('status')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="assigned_id"
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" @if(old('assigned_id') == '$user->id') selected @endif>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
